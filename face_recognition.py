...
#1st image
        imgtop_img=Image.open(r"images\18.jpg")
        imgtop_img=imgtop_img.resize((650,700),Image.BICUBIC)
        self.photoimgtop_img=ImageTk.PhotoImage(imgtop_img)

        f_lbl=Label(self.root,image=self.photoimgtop_img)
        f_lbl.place(x=0,y=55,width=650,height=700)
#2nd image 
        imgright_img=Image.open(r"images\19.jpg")
        imgright_img=imgright_img.resize((950,700),Image.BICUBIC)
        self.photoimgright_img=ImageTk.PhotoImage(imgright_img)

        f_lbl=Label(self.root,image=self.photoimgright_img)
    ...
    def face_recog(self):
        def draw_boundary(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x,y,w,h) in features:
                ...
                if confidence>77:
                    cv2.putText(img,f"Student id:{n}",(x,y-85),cv2.FONT_HERSHEY_COMPLEX,0.8,(255,255,255),3)
